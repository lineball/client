import { ADD_MOVE, AddMoveAction, INIT, REVERT_MOVE, REVERT_TURN } from './actions';
import { Action } from '../index';
import { Dot, Move, Path, Player, Size, Turn } from '../../game/defs';
import { Config, DefaultFieldConfig, initializeDots, initializePaths, initializeTurns } from './init';
import { getCurrentField, getCurrentTurn } from './selectors';
const initialState: GameState = ((config: Config = DefaultFieldConfig) => {
  const dots = initializeDots(config);
  const paths = initializePaths(dots);
  const turns = initializeTurns();
  return { config, dots, paths, turns };
})(DefaultFieldConfig);
...
const addMove = (state: GameState, action: Action) => {};
